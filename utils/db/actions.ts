  ...
  try {
    const rewards = await db
      .select({
        ...
        totalPoints: sql`SUM(${Transactions.amount})`.as('totalPoints'),
        userName: Users.name,
      })
      .from(Transactions)
      .leftJoin(Users, eq(Transactions.userId, Users.id)) // Join with Users table
      .groupBy(Transactions.userId, Users.name) // Group by userId and userName
      .orderBy(desc(sql`SUM(${Transactions.amount})`)) // Order by totalPoints in descending order
      .execute();

    return rewards.map(reward => ({
      ...reward,
      totalPoints: parseFloat(reward.totalPoints), // Ensure points are in number format
    }));
  } catch (error) {
